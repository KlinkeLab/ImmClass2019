...
############# Description:    This Script is Used to Prepare Data and Normalize It
############# Author:         Arezo Torang
############# Date:           13-Nov-2018
###########################################################################################-
###Reading Data-----
rm(list = ls())
Total <- read.table("Total.txt", sep = "\t", quote = "",stringsAsFactors=FALSE)
Total_Th <- read.table("Total_Th.txt", sep = "\t", quote = "",stringsAsFactors=FALSE)
####Keep Genes That Have >5 Values for at Least 5 Samples in Immune Cells----------
...
write.table(MainData, file = "MainData.txt", sep = "\t", quote = FALSE)

####Keep Genes That Have >5 Values for at Least 4 Samples in T helper Cells---------
ZeroGenes=data.frame()
Main=data.frame()
Total_0<-Total_Th
Total_0[is.na(Total_0)] <- 0
...
write.table(MainData_Th, file = "MainData_Th.txt", sep = "\t", quote = FALSE)

####Separation of Testing & Training Samples(Train:108,Test:77)(Train_Th:31,Test_Th:17)----------------------
TrainData<-MainData[,c(1:12,23:32,43:52,63:72,83:92,103:112,117,119:124,
                       127,129, 131, 135:136, 138:139, 141:142,144:146,
                       149:151,153, 155:157, 159:161, 163:165, 167:169,
                       171:173,175:176,178:179,181:182,184:185,187,188)]
write.table(TrainData,"TrainData.txt") #Training Data For Immune Cells

TestData<-MainData[,-c(3:12,23:32,43:52,63:72,83:92,103:112,117,119:124,
                       127,129,131, 135:136, 138:139, 141:142, 144:146,
                       149:151,153, 155:157, 159:161, 163:165, 167:169,
                       171:173,175:176,178:179,181:182,184:185,187,188)]
...
write.table(TrainData_Th,"TrainData_Th.txt") #Training Data For T helper Cells
...
####Normalization----------------
####Transcript Length & GC content for Immune Cell Samples (Using GC Content for Normalization)
####https://git.embl.de/rogon/predoc_2017/blob/master/Practical/mart_export.txt
Gene_Lenght <- read.table("mart_export.txt", header = TRUE, sep = "\t", dec = ".")
...
write.table(Repeat_Free, file = "Length_GC.txt", sep = "\t", quote = FALSE)
...
colnames(Repeat_Free) <- c("Genes", "Transcript_Length", "GC_Content")
Repeat_Free <- Repeat_Free[order(Repeat_Free[,1]),]
write.table(Repeat_Free, file = "Length_GC_Th.txt", sep = "\t", quote = FALSE)
...
write.table(Normalized_TrainData, file = "Normalized_TrainData.txt", sep = "\t", quote = FALSE)
...
write.table(Normalized_TestData, file = "Normalized_TestData.txt", sep = "\t", quote = FALSE)



####Training Samples for T helper Cells
boxplot(as.matrix(log2(TrainData_Th_Length_GC[,3:33]+1)))
dataWithin <- withinLaneNormalization(x=as.matrix(TrainData_Th_Length_GC[,3:33]),
                                    y=TrainData_Th_Length_GC[,35], offset=FALSE,
                                    round=TRUE, which = "full")
boxplot(log2(dataWithin+1))
dataNorm <- betweenLaneNormalization(dataWithin, which = "full")
boxplot(log2(dataNorm+1))
...
write.table(Normalized_TestData_Th, file = "Normalized_TestData_Th.txt", sep = "\t", quote = FALSE)
