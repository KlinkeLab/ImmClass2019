...
##Class Labels: 1:B Cell, 2:CD4, 3:CD8, 4:Mono, 5:Neu, 6:NK, 7:DC, 8:M1, 9:M2, 10:MQ
...
y<-rbind(ones(10, ncol = 1), 2*ones(10, ncol = 1), 3*ones(10, ncol = 1),
         4*ones(10, ncol = 1),5*ones(10, ncol = 1),6*ones(10, ncol = 1),
         1,7*ones(6, ncol = 1),4 ,5 ,6 ,8 ,8 ,9 ,9 ,2 ,2 ,3 ,3 ,3,10,10,
         10,6,ones(3, ncol = 1),2*ones(3, ncol = 1),3*ones(3, ncol = 1),
         4*ones(3, ncol = 1),6*ones(3, ncol = 1),7,7,4,4,8,8,9,9,10,10)
...
write.table(Selected_Genes, file = "Selected_Genes.txt", sep = "\t", quote = FALSE)
Coef<-coef(Lasso,s=0.0001)
Coef<-data.frame(Coef[["1"]]@x,Coef[["2"]]@x,Coef[["3"]]@x,Coef[["4"]]@x,
                 Coef[["5"]]@x,Coef[["6"]]@x,Coef[["7"]]@x,Coef[["8"]]@x,
                 Coef[["9"]]@x,Coef[["10"]]@x)
...
Coef_Th<-data.frame(Coef_Th[["1"]]@x,Coef_Th[["2"]]@x,Coef_Th[["3"]]@x,
                    Coef_Th[["4"]]@x,Coef_Th[["5"]]@x)
